...
{
  int case_suiv_x, case_suiv_y;

  position_devant(r, &case_suiv_x, &case_suiv_y);

  return est_case_libre(t, case_suiv_x, case_suiv_y) || (case_suiv_x == 0) || (case_suiv_y == 0) || case_suiv_x == t->largeur || case_suiv_y == t->hauteur;
}
...
int main(int argc, char **argv)
{
  ...
  printf("\nEntrer une action ([a]vancer, [g]auche, [d]roite, [f]in : ");
  scanf(" %c", &c);
  while (c != 'f')
  {
